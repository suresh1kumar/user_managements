...
import com.techouts.app.repository.UserRepositorys;
...
public class UserAccountController {
	...
	public ModelAndView registerUser(ModelAndView modelAndView, User user) {
		...
		User existingUser = userRepositorys.findByEmailIdIgnoreCase(user.getEmailId());
		if(existingUser != null) {
			...
			+"http://localhost:2021/confirm-account?token="+confirmationToken.getConfirmationToken());
	...
	public ModelAndView confirmUserAccount(ModelAndView modelAndView, @RequestParam("token")String confirmationToken) {
		...
		if(token != null)
		{
			User user = userRepositorys.findByEmailIdIgnoreCase(token.getUser().getEmailId());
	...
	public ModelAndView loginUser(ModelAndView modelAndView, User user) {
		...
		if(existingUser != null) {
			...
			if (encoder.matches(user.getPassword(), existingUser.getPassword())){
				System.out.println("encoder.matches(user.getPassword()2222==>:"+encoder.matches(user.getPassword(),existingUser.getPassword()));
				System.out.println("User.getPassword()222==>:"+user.getPassword());
...
/*	@PostMapping(consumes = "application/json", produces = "application/json", path = "/forgot-password1")  
    public ResponseEntity<?> forgotUserPasswordSendMails( @RequestBody User user) throws Exception
    {
	...
	public ModelAndView forgotUserPassword(ModelAndView modelAndView, User user) {
		...
		if(existingUser != null) {
			...
			+"http://localhost:2021/confirm-reset?token="+confirmationToken.getConfirmationToken());
			emailSenderService.sendEmail(mailMessage);
	...
	public ModelAndView resetUserPassword(ModelAndView modelAndView, User user) {
		...
		if(user.getEmailId() != null) {
			...
			User tokenUser = userRepositorys.findByEmailIdIgnoreCase(user.getEmailId());
